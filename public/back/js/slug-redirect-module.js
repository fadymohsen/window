...
class SlugRedirectController {
    constructor() {
        this.create_form = document.querySelector("form#create-slug-redirect-form")
        if(this.create_form) {
            this.create_form.addEventListener("submit", this.create.bind(this))
        }

        this.edit_form = document.querySelector("form#edit-slug-redirect-form")
        if(this.edit_form) {
    ...
    async remove(id)
    {
        const response = await request(`/dashboard/slug-redirects/${id}`, 'DELETE')
        if(response.success) {
    ...
    async create(e) {
        ...
        const response = await request('/dashboard/slug-redirects', 'POST', formData)
        if(response.success) {
            this.show_success(response.data.message)
    ...
    async edit(e) {
        ...
        const response = await request(`/dashboard/slug-redirects/${id}`, 'PUT', formData)
...
export default new SlugRedirectController();
