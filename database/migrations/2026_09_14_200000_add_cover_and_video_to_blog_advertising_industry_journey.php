...
{
    private string $slug = 'advertising-industry-journey-riyadh';
    private string $coverPath = 'blogs/covers/advertising-industry-journey-riyadh.webp';
...
HTML;
    ...
    public function down(): void
    {
        ...
        if ($blog->cover === $this->coverPath) {
            DB::table('blogs')->where('id', $blog->id)->update(['cover' => 'blogs/covers/placeholder-advertising-industry-journey.webp']);
        }
    ...
    {
        $source = base_path('resources/blog-assets/advertising-industry-journey-riyadh-window.png');
        if (!is_file($source)) {
            return;
        }
    ...
    private function insertVideoAfterFirstParagraph(int $blogId, string $locale): void
    {
        ...
        if (!$translation || str_contains($translation->description, 'youtube.com/embed/Kc1H5w3BviQ')) {
            return;
        }
