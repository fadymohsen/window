...
HTML;
    ...
    public function up(): void
    {
        ...
        foreach (['ar', 'en'] as $locale) {
            ...
            // Prepend video before the content (before the first <h2>)
            $description = $this->videoEmbed . "\n\n" . $description;
        ...
        foreach (['ar', 'en'] as $locale) {
            ...
            if ($translation && str_contains($translation->description, $this->videoEmbed)) {
                DB::table('blog_translations')
                    ->where('blog_id', $blog->id)
                    ->where('locale', $locale)
                    ->update(['description' => trim(str_replace($this->videoEmbed, '', $translation->description))]);
            }
        }
    }
};
