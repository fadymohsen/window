...
{
    private string $slug = 'saudi-national-day-96-gifts-window';
    private string $coverPath = 'blogs/covers/saudi-national-day-96-gifts-window.webp';
...
<div style="max-width: 360px; margin: 24px auto;">
    <div style="position: relative; padding-bottom: 177.78%; height: 0; border-radius: 12px; overflow: hidden;">
        <iframe src="https://www.youtube.com/embed/Uqd71V2JKVo" title="Window Advertising Agency"
...
HTML;
    ...
    public function down(): void
    {
        ...
        if ($blog->cover === $this->coverPath) {
            DB::table('blogs')->where('id', $blog->id)->update(['cover' => 'blogs/covers/placeholder-national-day-96.webp']);
        }
    ...
    {
        $source = base_path('resources/blog-assets/saudi-national-day-96-gifts-window.jpg');
        if (!is_file($source)) {
            return;
        }
    ...
    private function insertVideoAfterFirstComponent(int $blogId, string $locale): void
    {
        ...
        if (!$translation || str_contains($translation->description, 'youtube.com/embed/Uqd71V2JKVo')) {
            return;
        }
